...
export  default class UserModel extends BaseModel   {
    ...
    async getAllCities(query:string){
        return await this._executeQuery(`select id as value, name as label from address_city ${query}`,[])
    }
    ...
    async getCitiesByState(state_id:number, query : string ){
        return await this._executeQuery(`select act.id as value, act.name as label from biofuel.address_city act inner join biofuel.address_state ast ON  act.state_id = ast.id where ast.id = ? ${query}`,[state_id])
    }
