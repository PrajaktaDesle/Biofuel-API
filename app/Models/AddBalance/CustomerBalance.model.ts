...
export class CustomerBalanceModel extends BaseModel {
    ...
    async updateCustomerBalance(sqlConnection : Connection, customer_id:number,balance:number) {
        return await this._executeQueryTransaction(sqlConnection, "update customer_balance set balance = ? where customer_id = ?", [balance,customer_id]);
    }
}