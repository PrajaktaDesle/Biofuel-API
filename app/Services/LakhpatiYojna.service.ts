...
import {LakhpatiYojnaModel} from "../Models/LakhpatiYojna/LakhpatiYojna.model";


async function createLakhpatiYojna(data: any) {
    try {
        console.log(data)
        let LakhpatiYojnaDetails = await new LakhpatiYojnaModel().get_maturity_amount(data);
        ...
        let customer = await new LakhpatiYojnaModel().createLakhpatiYojna(data);
...
async function fetchByCustomerId(customer_id: any, tenant_id: any) {
    try {
        ...
        let sukanyaYojnaList = await new LakhpatiYojnaModel().fetchByCustomerId(customer_id, tenant_id);
