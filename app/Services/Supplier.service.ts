...
const createSupplier = async (req:any) =>{
    try{
        ...
        let profile = {"aadhaar_no":fd.aadhaar_no,"pan_no":fd.pan_no,"gstin_no":fd.gstin_no,"msme_no":fd.msme_no,"user_id":user_id, "comment":fd.comment, "payment_term": fd.payment_term, "grade": fd.grade}
...
const fetchSupplierById = async (id: any) => {
    try {
        ...
        supplier[0].raw_material = [];
        supplier[0].raw_material.push(rm);
...
const updateSupplierDetails = async (req:any) =>{
    try{
        ...
        let s3Images:any = {};
        if(fl.aadhaar_img !== undefined && fl.aadhaar_img !== null && fl.aadhaar_img !== ""){
        if(isFileNotValid(fl.aadhaar_img.mimetype))throw new Error("Only .png, .jpg and .jpeg format allowed! for aadhaar_img");else{s3Images.aadhaar_img=fl.aadhaar_img;}}
        if(fl.pan_img !== undefined && fl.pan_img !== null && fl.pan_img !== ""){
        if(isFileNotValid(fl.pan_img.mimetype))throw new Error("Only .png, .jpg and .jpeg format allowed! for msme_img! for pan_img");else{s3Images.pan_img=fl.pan_img;}}
        if(fl.gstin_img !== undefined && fl.gstin_img !== null && fl.gstin_img !== ""){
        if(isFileNotValid(fl.gstin_img.mimetype))throw new Error("Only .png, .jpg and .jpeg format allowed! for msme_img! for gstin_img");else{s3Images.gstin_img=fl.gstin_img;}}
        if(fl.msme_img !== undefined && fl.msme_img !== null && fl.msme_img !== ""){
        if(isFileNotValid(fl.msme_img.mimetype))throw new Error("Only .png, .jpg and .jpeg format allowed! for msme_img");else{s3Images.msme_img=fl.msme_img}}
